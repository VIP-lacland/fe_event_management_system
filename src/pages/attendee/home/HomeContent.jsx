...
const HomeContent = ({
  ...
  const firstEventNumber =
    filteredEvents.length === 0 ? 0 : (currentPage - 1) * eventsPerPage + 1;
  const lastEventNumber = Math.min(
    ...
    <section className="homepage-content" id="events">
      ...
      <div className="event-grid">
        {paginatedEvents.map((event) => {
          const thumbStyle = {
            backgroundImage:
              categoryImages?.[event.category] || "url('/images/banner.png')",
            ...
            <article className="event-card" key={event.id}>
              ...
              <div className="event-content">
                ...
                <p className="event-description">
                  {event.description ||
                    "Sự kiện hấp dẫn đang chờ bạn khám phá."}
                </p>
                <div className="event-capacity">
                  <span>Capacity</span>
                  <strong>
                    {event.capacity?.toLocaleString?.() ?? event.capacity}{" "}
                    people
                  </strong>
                </div>
                <div className="event-footer">
                  ...
                  <Link
                    to={`/event/${event.id}`}
                    className="event-action btn-chi-tiet"
                  >
                    Event Details
                  </Link>
                </div>
      ...
      {!loading && !error && filteredEvents.length > 0 && (
        <div className="pagination-bar" aria-label="Event pagination">
          <p className="pagination-summary">
            Showing {firstEventNumber}-{lastEventNumber} of{" "}
